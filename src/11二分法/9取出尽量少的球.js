...
// 翻译：只要是总和>=sum，那么就需要取出小球，一直取，取到总和小于sum为止。获取最大的桶的容量，可以让剩下的球<sum，返回取出的小球数组。
// 容量最小是0，最大是max(...arr)
...
void async function(){
    ...
    console.log(getRes(sum,n,arr))
}()

const check = (sum,arr,target)=>{
    let remain = 0;
    for(let v of arr){
        remain += target>=v ? v : target;
    }
    return remain <= sum;
}
const getRes = (sum,n,arr)=> {
    let count = arr.reduce((a, b) => a + b);
    let res = 0;
    if (count > sum) {
//     取出球 二分计算合适的容量 res为最大的桶容量
        let left = 0, right = Math.max(...arr);
        while (left <= right) {
            let mid = Math.floor((left + right) / 2);
            // 如果符合剩余总和小于SUM，求更大的容量
            if (check(sum,arr,mid)) {
                res = mid;
                left = mid + 1;
            } else {
                right = mid - 1;
        ...
        // 最后返回，从每个桶中取出的值 本身大小-桶容量
        return arr.map(v => v < res ? 0 : v - res);
    }else{
        return [];
    }
}
...
/*
// 思路: 二分法check  check计算剩下球是否满足条件，求更大值
