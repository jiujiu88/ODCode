...
void async function(){
    ...
    console.log(getRes(x,y,cntx,cnty))
}()

const check = (x,y,cntx,cnty,mid)=>{
    let noX = Math.floor(mid/x);
    let noY = Math.floor(mid/y);
    let noXY = Math.floor(mid/(x*y));
//    noY都去x国，noX都去y国   剩下的mid-noX-noY的人两个国家都可以去,
    let remain = Math.max(0,cntx-noY)+Math.max(0,cnty-noX);
    return remain<=mid-noX-noY;
}
// 思路:二分法
const getRes = (x,y,cntx,cnty)=>{
    let left = cntx+cnty,right = 1e9;
    while(left<=right){
