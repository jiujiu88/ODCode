...
const binarySearch = (target,weights)=>{
...
const getRes = (weights,n)=>{
    // 升序,用pop取最重的(shift效率低)
    weights.sort((a,b)=>a-b);
    while(weights.length>=3){
        const z = weights.pop(),y = weights.pop(),x = weights.pop();
        // 记录剩下还有多少没熔掉
        let diff = z-x;
        // let diff = Math.abs((z - y) - (y - x));
        // 将它插入原数组中,这样能保持原数组是有序的
        if(diff>0){
            // 二分快速查找位置
            let idx = binarySearch(diff,weights);
            weights.splice(idx, 0, diff);
        }
    }
//     如果剩余两块，返回较大的重量（若两块重量相同，返回任意一块皆可）；如果只剩下一块，返回该块的重量；如果没有剩下，就返回 0。
//     weight本来就是升序的,如果两块,返回weights[1]
    if(weights.length===0){
        return 0;
